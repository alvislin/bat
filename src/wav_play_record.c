...
void *play(void *bat_param)
{
	...
	if (NULL != pa_bat->device) {
		err = snd_pcm_open(&sndpcm.handle, pa_bat->device, SND_PCM_STREAM_PLAYBACK, 0);
		if (err < 0) {
...
fail_exit:
	if (fp) fclose(fp);
	if (sndpcm.buffer) free(sndpcm.buffer);
	if (sndpcm.handle) snd_pcm_close(sndpcm.handle);
	retval_play = 1;
	pthread_exit(&retval_play);
}
...
void *record(void *bat_param)
{
	...
	pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);
        ...
        memset(&sndpcm, 0, sizeof(sndpcm));
        if (NULL != pa_bat->device) {
                err = snd_pcm_open(&sndpcm.handle, pa_bat->device, SND_PCM_STREAM_CAPTURE, 0);
                if (err < 0) {
                        fprintf(stderr, "Unable to open pcm device: %s\n", snd_strerror(err));
                        goto fail_exit;
                }
	...
	while(count > 0) {
		size = (count <= sndpcm.period_bytes) ? count : sndpcm.period_bytes;
		frames = size * 8 / sndpcm.frame_bits;
		offset = 0;
		while (frames > 0) {
        		err = snd_pcm_readi(sndpcm.handle, sndpcm.buffer + offset, frames);
